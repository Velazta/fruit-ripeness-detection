...
def extract_features(image_path):
    ...
    mean_rgb = np.mean(img_rgb_res, axis=(0, 1))
    std_rgb = np.std(img_rgb_res, axis=(0, 1))
    mean_lab = np.mean(img_lab_res, axis=(0, 1))
    std_lab = np.std(img_lab_res, axis=(0, 1))
...
# ==================== MAIN PIPELINE ====================
def main():
    print("=== TAHAP 1: MEMUAT PATH DATASET ===")
    all_train_paths = {}
    all_test_paths = {}
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(ARTIFACT_DIR, 'confusion_matrices.png'), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(ARTIFACT_DIR, 'decision_boundaries.png'), dpi=150)
    plt.close()
